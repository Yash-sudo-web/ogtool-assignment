...
async function scrapeWithAxios(url: string): Promise<Omit<IContent, 'team_id' | 'user_id'> | null> {
  try {
...
async function scrapeWithPuppeteer(url: string): Promise<Omit<IContent, 'team_id' | 'user_id'>> {
...
function extractContent($: cheerio.CheerioAPI, url: string): Omit<IContent, 'team_id' | 'user_id'> {
  // Extract title
...
function determineContentType($: cheerio.CheerioAPI, url: string): 'blog' | 'podcast_transcript' | 'call_transcript' | 'linkedin_post' | 'reddit_comment' | 'book' | 'other' {
  const urlLower = url.toLowerCase();
  const bodyText = $('body').text().toLowerCase();
  
  // Check URL patterns
  if (urlLower.includes('linkedin.com')) return 'linkedin_post';
  if (urlLower.includes('reddit.com')) return 'reddit_comment';
  if (urlLower.includes('podcast') || urlLower.includes('transcript')) return 'podcast_transcript';
  if (urlLower.includes('call') || urlLower.includes('meeting')) return 'call_transcript';
  if (urlLower.includes('book') || urlLower.includes('pdf')) return 'book';
  
  // Check content patterns
  if (bodyText.includes('podcast') || bodyText.includes('transcript')) return 'podcast_transcript';
  if (bodyText.includes('call') || bodyText.includes('meeting')) return 'call_transcript';
...
function calculateContentScore(content: string): number {
  ...
  if (content.length < 50) score -= 100;
